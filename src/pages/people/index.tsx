...
import { getPeople, deletePeopleById } from 'apiSdk/people';
import { PeopleInterface } from 'interfaces/people';

type ColumnType = ColumnDef<PeopleInterface, unknown>;
...
export function PeopleListPage(props: PeopleListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getPeople({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'role.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PeopleInterface>>(
    () => `/people?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PeopleInterface) => {
    if (hasAccess('people', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/people/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/people/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('people', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
          ...
          {hasAccess('people', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="people" />
        </Flex>

        {hasAccess('people', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
